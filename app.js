...
function updateUpcomingActivities() {
    ...
    Storage.getTareas().filter(t => !t.completada && t.fecha === today).forEach(tarea => {
        activities.push({
...
function loadMedicamentos() {
    ...
    const limits = Storage.checkLimits();
...
function deleteMedicamento(id) {
    if (confirm('¿Estás seguro de que quieres eliminar este medicamento?')) {
        Storage.deleteMedicamento(id);
        loadMedicamentos();
        loadDashboard();
    }
}

function saveMedicamento(event) {
    ...
    if (id) {
        Storage.updateMedicamento(id, medicamento);
    } else {
        Storage.addMedicamento(medicamento);
    }
...
function registrarTomaMedicamento(id) {
    const medicamento = Storage.getMedicamentos().find(m => m.id === id);
    if (!medicamento) return;
...
function renderCitasList(filter = 'todas') {
    const citas = Storage.getCitas();
...
function saveCita(event) {
    ...
    if (id) {
        Storage.updateCita(id, cita);
    } else {
        Storage.addCita(cita);
    }
...
function loadTareas(filter = 'todas') {
...
function toggleTareaCompletada(id) {
    const tarea = Storage.getTareas().find(t => t.id === id);
    if (tarea) {
        Storage.updateTarea(id, { completada: !tarea.completada });
        loadTareas();
        loadDashboard();
    }
}
...
function saveTarea(event) {
    ...
    if (id) {
        Storage.updateTarea(id, tarea);
    } else {
        Storage.addTarea(tarea);
    }
...
function loadContactos(filter = 'todos') {
    const contactos = Storage.getContactos();
...
function switchContactoTab(tab) {
    loadContactos(tab);
}
...
function deleteContacto(id) {
    if (confirm('¿Estás seguro de que quieres eliminar este contacto?')) {
        Storage.deleteContacto(id);
        loadContactos();
    }
}

function saveContacto(event) {
    event.preventDefault();
    ...
    if (id) {
        Storage.updateContacto(id, contacto);
    } else {
        Storage.addContacto(contacto);
    }
